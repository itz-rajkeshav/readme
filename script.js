...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const containerHeight =
    document.querySelector(".code-container").clientHeight;
  ...
  function addLine() {
    ...
    const keywordProbability = 0.3; // 30% probability to replace a block with a keyword
    ...
    for (let i = 0; i < numberOfWords; i++) {
      if (Math.random() < keywordProbability) {
        ...
        word.style.width = isLongerLine
          ? `${Math.random() * 60 + 50}px`
          : `${Math.random() * 20 + 30}px`; // Random width for each word, longer lines more probable
